#!/usr/bin/env python3
"""
Lightweight preview: ring-detection panel localization + TrackerNano
(deep-learning, lightweight ONNX-based) tracking, instead of TrackerMIL/
TrackerVit. No digit cropping, no OCR/segment decode, no Excel output -
just draws the tracked green box on each sampled frame and saves it, for
fast visual comparison against the MIL preview (test_ring_detection/) and
the VitTrack preview (test_vit_tracker/).

Usage (from inside nestle_syringie/deep_learning_tracker/):
    ../../.venv/bin/python nano_tracker_preview_only.py <video1> <video2> ...

Output: test_nano_tracker/<video_key>/annotated_frames/frame_NNNNNN.png
...
OUTDIR = Path(__file__).parent / "test_nano_tracker"
...
def get_roi(video_path, cache):
    ...
    present (keeps the comparison to MIL/VitTrack fair - same starting
    box), else detect fresh via ring detection."""
...
def main():
    videos = sys.argv[1:]
    if not videos:
        print("usage: nano_tracker_preview_only.py <video1> [video2 ...]")
        return
    cache = load_cache()
