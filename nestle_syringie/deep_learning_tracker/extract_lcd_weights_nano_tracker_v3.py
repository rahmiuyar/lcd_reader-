#!/usr/bin/env python3
"""
v3: same as v2 (ring detection + TrackerNano + full pipeline + split-wide-
box fix), but with split_wide_boxes rewritten - v2's median-of-siblings
approach turned out unreliable in two ways, found by re-checking whether
the fix actually fired on the exact frame it was designed for:

  1. With only 2 boxes in a frame (one merged pair + one real single
     digit, e.g. lvl3_60mL-3 frame 18: widths 347 and 179), the median of
     JUST THOSE TWO is pulled up to 263 by the merged box itself, so
     347 < 263*1.6 never re-triggers the split - the fix silently didn't
     fire on its own motivating case (verified visually: v2's boxes.png
     for that frame still shows one merged box).
  2. Sibling-relative comparison is contaminated by digit '1', whose ink
     bounding box is much narrower than other digits' (no left segments) -
     a frame containing an unmerged '1' next to normal digits skews the
     median down, so a normal (unmerged) '8' or '0' can look "1.6x the
     median" and get wrongly split. This is the likely cause of v2's
     regression on lvl1_20mL-2/lcd_reader_20mL-2 (84%->82%, i.e. new
     failures appeared that weren't there in v1).

Fix: judge each box independently by its own width/height ratio instead of
comparing to sibling boxes in the same frame - avoids both problems since
it needs no other box to reference. Measured across 381 real digit boxes
from test_nano_full/ocr_debug (a random sample spanning all 8 videos):
single real digits (including '1') cluster at width/height <= 0.73 (95th
pct = 0.57); confirmed 2-digit merges measured at 0.87-0.99. Clean gap
in between, so a fixed threshold of 0.78 separates them reliably without
needing per-frame calibration.
"""
import sys
...
OUTDIR = Path(__file__).parent / "test_nano_full_v3"
...
def main():
    ...
    print("\n=== Batch summary (TrackerNano + per-box ratio split fix) ===")
