...
v7: fixes a THIRD bug, found returning to iddsi2-20ml-1 (still reading a
genuinely-displayed "0.00" as "8.40" even after the above fixes - tracking
and cropping both confirmed correct, so the bug had to be in classify_digit
itself). Root cause: this video's digit boxes aren't all the same
proportions - measured on frame 0's three digits, width/height ratios of
154/406=0.38, 175/406=0.43, 187/378=0.49, a real gradient across the
display (likely mild perspective/keystone from the handheld phone angle,
not a tracking or crop bug - the box for each digit is a tight, correctly-
split crop around genuinely narrower ink). classify_digit's 'g' (middle-
bar) segment sampled x=0.15-0.85 - wide enough that on the narrowest digit,
the italic-shear-tapered tips of the LEFT/RIGHT strokes (present on every
digit, tapering to a point where they'd meet a middle bar) poked into the
sampling band and read as a false middle segment, turning a clean hollow
"0" (pattern 1,1,1,0,1,1,0) into "8" (1,1,1,0,1,1,1) - one segment off,
close enough to be the winning nearest-match. Narrowed 'g' to x=0.25-0.75:
fixes this digit (verified: reads 0, not 8) without changing any digit
that has a REAL middle bar (2,3,4,5,6,8,9 all still fill the narrower
center band, since a genuine middle stroke spans the full width, not just
the tapered edges).
"""
import sys
...
from new_template_method_ring_detect import (
    ...
    preprocess_digits_area, detect_panel_in_frame,
    calibrate_rotation_and_crop, DIGIT_LOOKUP,
...
# Narrowed from the base script's (0.15, 0.85) - see module docstring for
# the measured justification (tapered stroke tips on narrower-than-usual
# digit boxes were poking into the middle-segment sample band).
G_SEGMENT_X_RANGE = (0.25, 0.75)
...
def classify_digit(digit_img):
    ...
    g = filled(G_SEGMENT_X_RANGE[0], 0.44, G_SEGMENT_X_RANGE[1], 0.56)
...
NANO_BACKBONE_PATH = Path(__file__).parent / "deep_learning_tracker" / "models" / "nanotrack_backbone.onnx"
NANO_HEAD_PATH = Path(__file__).parent / "deep_learning_tracker" / "models" / "nanotrack_head.onnx"
OUTDIR = Path(__file__).parent / "test_nano_full_v7"
...
def main():
    ...
    print("\n=== Batch summary (TrackerNano + split fix + ring detection + narrow g-segment) ===")
